...
def get_c2w_nerfconv(qvec, tvec):
    R = qvec2rotmat(qvec)
    ...
    c2w = np.linalg.inv(c_T_w)
    c2w[0:3,2] *= -1 # flip the y and z axis
    c2w[0:3,1] *= -1
    # for Aachen, axis in the 3D model are different,
    # so comment these 2 lines below
...
def read_cameras_intrinsics(path):
    """
    ...
    with open(path, "r") as fid:
        while True:
            ...
            if len(line) > 0 and line[0] != "#":
                ...
                cameras.append(Camera(id=path, model=model,
...
def read_model_nopoints(path, ext=""):
...
def write_cameras_text(cameras, path, header=True):
    """
    see: src/base/reconstruction.cc
        void Reconstruction::WriteCamerasText(const std::string& path)
        void Reconstruction::ReadCamerasText(const std::string& path)
...
def write_model_nopoints(cameras, images, path, ext=".txt"):
    if ext == ".txt":
        write_cameras_text(cameras, os.path.join(path, "cameras" + ext))
        write_images_text(images, os.path.join(path, "images" + ext))
    else:
